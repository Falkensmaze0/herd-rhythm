...
The application now includes a secure, modular authentication system with the following features:

- **User Registration**: Create new accounts via API-driven registration.
- **Login and Logout**: Secure login with session management using HttpOnly cookies and localStorage tokens.
- **Password Reset**: Full password reset flow with confirmation.
- **Session Validation**: Automatic session checks and role-based access control.
- **Role-Based Redirection**: Users are redirected to appropriate landing pages based on their roles (e.g., admin, user).
- **API-First Architecture**: All authentication actions are handled through dedicated API endpoints (`/api/auth/*`), ensuring scalability and security.
...
| `/api/auth/login` | `POST` | Authenticate a user and establish a session. |
| `/api/auth/logout` | `POST` | End the current user session. |
| `/api/auth/reset` | `POST` | Initiate a password reset. |
| `/api/auth/reset/confirm` | `POST` | Confirm and complete a password reset. |
| `/api/auth/session` | `GET` | Validate and retrieve current session details. |
